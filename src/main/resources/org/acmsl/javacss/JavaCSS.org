...
Now it's a little more readable, and it's parsing the import correctly with no complaints.
Let's hope it's not too expensive in terms of performance. Clearly, we should reuse the lexer and tokens from the initial parsing stage. We'll fix it
when time is ready.
